...
std::string fileTimeToString(uint64_t filetime)
{
    ...
    oss << std::setfill('0') << (timeinfo.tm_year + 1900) << "." << std::setw(2)
        << (timeinfo.tm_mon + 1) << "." << std::setw(2) << timeinfo.tm_mday << " " << std::setw(2)
        << timeinfo.tm_hour << ":" << std::setw(2) << timeinfo.tm_min << ":" << std::setw(2)
        << timeinfo.tm_sec;
    return oss.str();
}
...
std::string utf16leToUtf8(const uint8_t* data, size_t offset, size_t maxBytes)
{
    ...
    size_t len = 0;
    while (offset + len + 1 < maxBytes && (data[offset + len] != 0 || data[offset + len + 1] != 0))
    {
        len += 2;
        if (len > 0x1000)
            break; // safety limit
    ...
    try
    {
        ...
        {
            char16_t ch = static_cast<char16_t>(data[offset + i] | (data[offset + i + 1] << 8));
            utf16.push_back(ch);
        }
...
std::optional<Record> parseSccaFormat(const uint8_t* data, size_t dataSize,
                                      const std::string& pfFileName)
{
    ...
    if (volInfoCount > 0 && volInfoOffset + 8 <= dataSize)
    {
        ...
        {
            result.volumeCreateTime =
                fileTimeToString(readUint64LE(data, volDataOffset + 8)); // creation time at +8
        }
    }
    ...
    size_t stringTableEnd =
        std::min(static_cast<size_t>(stringTableOffset + stringTableSize), dataSize);
    ...
    for (uint32_t i = 0; i < fileMetricsCount && fileMetricsOffset + fileMetricSize <= dataSize;
         ++i)
    {
        ...
        {
            std::string fullPath =
                utf16leToUtf8(data, stringTableOffset + strOffset, stringTableEnd);
            if (!fullPath.empty())
            {
...
std::optional<Record> parseMamFormat(const uint8_t* data, size_t dataSize,
                                     const std::string& pfFileName)
{
    ...
    // body][run_count:4][last_run:8][history:7*8][volume_serial:4][volume_time:8]
